...
一款面向高考考生的 AI 志愿规划应用。通过自然对话式测评收集分数、位次、选科、性格、城市偏好、家庭资源等多维信息，结合大语言模型与实时网络搜索，为每位考生生成个性化的志愿规划报告。支持支付宝 3 元解锁完整报告，适合独立开发者快速上线运营。
...
- **支付解锁**：完整报告需支付 3 元，集成 ZPAY 支付宝支付网关，真实支付后解锁。
...
| `XPAY_REPORT_AMOUNT` | 否 | `3.00` | 报告解锁金额 |
...
3. 报告解锁金额默认为 3 元，可通过 `XPAY_REPORT_AMOUNT` 修改。
